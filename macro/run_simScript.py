...
# Default sgoldstino parameters
theSGmass    = 1.0*u.GeV

"""
Probably need to add some more things here, like the HNL's, or make it more like the DP's.
"""
...
parser.add_argument("--Sgoldstino", dest="Sgoldstino", help="Generate sgoldstinos", required=False , action="store_true")
...
# Still do not know what parameters I need. Can probably just utilize couplings
#parser.add_argument("-sqrtf", "--breaking-parameter", dest="theBreakingParameter", help="breaking parameter \\sqrt(F) for the Sgoldstinos only", required=False, default=None)
...
if options.Sgoldstino: 
  HNL = False
  inputFile = "/eos/experiment/ship/data/Beauty/Cascade-run0-19-parp16-MSTP82-1-MSEL5-5338Bpot.root" #The production of sgoldstinos comes only from B's for now
if not options.theMass:
  if options.DarkPhoton: options.theMass   = theDPmass
  elif options.Sgoldstino: options.theMass = theSGmass
...
#sanity check
if (HNL and options.RPVSUSY) or (HNL and options.DarkPhoton) or (HNL and options.Sgoldstino) or (options.DarkPhoton and options.RPVSUSY) or (options.DarkPhoton and options.Sgoldstino) or (options.Sgoldstino and options.RPVSUSY): 
...
# read from external file
 ...
 """
 Should add the Sgoldstino option here
 if options.Sgoldstino:
  P8gen = ROOT.SGPythia8Generator()
  import pythia8sgoldstino_conf
  print('Generating SG events of mass %.3f GeV'%options.theSGmass)
  
  #Need to put the necessary coupling checks here
 
  P8gen.SetParameters("ProcessLevel:all = off")

 """
 if HNL or options.RPVSUSY or options.DarkPhoton or options.Sgoldstino: 
