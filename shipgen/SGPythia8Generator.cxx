...
// -----   Default constructor   -------------------------------------------
SGPythia8Generator::SGPythia8Generator() 
{
  //fHadDecay = false;
  fId         = 2212; // proton
  fMom        = 400;  // proton
  fSG         = 9900039;    // sgoldstino pdg code (99 because it is only for this and 39 because
                            // the goldstino is a component of the sgraviton, and the graviton code
                            // is 39)
                            // (perhaps not too accurate but we are freestyling here)
...
// -----   Default constructor   -------------------------------------------
Bool_t SGPythia8Generator::Init() 
{
  ...
  if (fextFile && *fextFile){
    ...
    */ 
    ...
    if (fdy) fPythia->settings.parm("PhaseSpace:mHatMin",fSGminM)
  ...
  Double_t root_ctau = pdgBase->GetParticle(fSG)->Lifetime();
  //fPythia->particleData.readString("4900023:useBreitWigner = false");
  if ( debug ){
    std::cout<<"Final particle parameters for PDGID " << fSG << ":" << std::endl;
    List(fSG);
  }
  if ( debug ){std::cout<<"tau root PDG database "<<root_ctau<< "[s] ctau root = " << root_ctau*3e10 << "[cm]"<<std::endl;}
  fctau = fPythia->particleData.tau0(fSG); //* 3.3333e-12
...
// -----   Passing the event   ---------------------------------------------
Bool_t SGPythia8Generator::ReadEvent(FairPrimaryGenerator* cpg)
{
   Double_t tp,tS,zp,xp,yp,zS,xS,yS,pz,px,py,e,w;  
   Double_t tm,zm,xm,ym,pmz,pmx,pmy,em;
   //Double_t td,zd,xd,yd;
   Int_t im;
// take SG decay of Pythia, move it to the SHiP decay region
// recalculate decay time 
// weight event with exp(-t_ship/tau_SG) / exp(-t_pythia/tau_SG) 

   int iSG = 0; // index of the chosen SG, also ensures that at least 1 SG is produced
   std::vector<int> dec_chain; // pythia indices of the particles to be stored on the stack
   std::vector<int> sgvec; // pythia indices of SG particles
   //bool hadDecay = false;
   do {
     ...
     */
     ...
     fnSGtot += iSG;
     if ( iSG == 0 ){
       //fLogger->Info(MESSAGE_ORIGIN,"Event without SG. Retry.");
       //fPythia->event.list();
       fnRetries+=1; // can happen if phasespace does not allow meson to decay to SG
     }else{

	/***********************************************************************
 * *****************************************************************************
 * ****************       NEED TO CHECK KYNEMATICS     *************************
 * *****************************************************************************
 * ****************************************************************************/


       //HERE WE GET iSG-1 since we want the last one in the SG case
	 ...
	 cpg->AddTrack(fSG, px, py, pz, xp/cm+dx,yp/cm+dy,zp/cm, 0,false,e,tp/cm/c_light,w); 
       }
       ...
       if (debug>1) std::cout << " ----> insert SG id " << i << " pdg=" << fSG << " pz = " << pz << " [GeV] zp = " << zp << " [mm] tp = " << tp << " [mm/c]" << std::endl;
       iSG = i; 
     }
   } while ( iSG == 0 ); // ----------- avoid rare empty events w/o any SG's produced
   
   if (fShipEventNr%100==0) {
     fLogger->Info(MESSAGE_ORIGIN,"ship event %i / pythia event-nr %i",fShipEventNr,fn);
   }
   fShipEventNr += 1;
   // fill a container with pythia indices of the SG decay chain
   if (debug>1) std::cout << "Filling daughter particles" << std::endl;
   //if (!hadDecay){
   for(int k=0; k<fPythia->event.size(); k++){
     // if daughter of SG, copy
     if (debug>1) std::cout <<k<< " pdg =" <<fPythia->event[k].id() << " mum " << fPythia->event[k].mother1() << std::endl;
     im =fPythia->event[k].mother1();
     while (im>0){
       if ( im == iSG ){break;} // pick the decay products of only 1 chosen SG
       // if ( abs(fPythia->event[im].id())==fSG && im == iSG ){break;}
     ...
     // go over daughters and store them on the stack, starting from 2 to account for SG and its mother
...
// -------------------------------------------------------------------------
void SGPythia8Generator::SetParameters(char* par)
{
  // Set Parameters
