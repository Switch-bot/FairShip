...
class SGPythia8Generator : public FairGenerator
{
 public:
  ...
  void SetSGId(Int_t id) { fSG = id; };
  Int_t GetSGId() { return fSG; };
  ...
  Double_t MinSGMass() { return fSGminM; };
  void SetMinSGMass(Double_t m){
    fSGminM = m;
  };

  void UseDeepCopy(){ fDeepCopy   = kTRUE; };
  Int_t nrOfRetries(){ return fnRetries; };
  //Int_t nrOfSG(){ return fnSGtot; };
 ...
 protected:
  ...
  Double_t fSGminM;    //Minimum mass, in GeV, for the SG produced in ffbar to DP QCD production.
  Double_t fLmin;      // m minimum  decay position z
  Double_t fLmax;      // m maximum decay position z
  Int_t fnRetries;     // number of events without any SG 
  //Int_t fnSGtot;       // total number of SG from multiple mesons in single collision
  Double_t fctau;      // sgoldstino lifetime 
