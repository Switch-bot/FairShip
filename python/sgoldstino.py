...
class sgoldstino:
    ...
    """NEED TO START REPLACING THINGS FROM HERE ON OUT"""




    def leptonicDecayWidth(self,lepton): # mSgoldstino in GeV
        """ Sgoldstino decay width into leptons, in GeV (input short name of lepton family) """
        ml = mass(lepton)
        #print 'lepton %s mass %.3e'%(lepton,ml)
        
        constant = (1./3.) * alphaQED * self.msgoldstino * pow(self.epsilon, 2.)
        if 2.*ml < self.msgoldstino:
