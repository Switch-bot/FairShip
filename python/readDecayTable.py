...
def addSgoldstinodecayChannels(P8gen,SG,conffile=os.path.expandvars('$FAIRSHIP/python/sgoldstinoDecaySelection.conf'), verbose=True):
    """
    Configures the SG decay table in Pythia8
    ...
    """
    isResonant = (P8gen.GetDPId()==4900023)# or P8gen.IsPbrem())
    Relic from the DP definition, where it is also defined as 4900023 from the hidden valley nomenclature
    """

    # First fetch the list of kinematically allowed decays
    allowed = SG.allowedChannels()
    # Then fetch the list of desired channels to activate
    wanted = load(conffile=conffile, verbose=verbose)
    # Add decay channels
    for dec in allowed:
        print('channel allowed:',dec)
        if dec not in wanted:
            print('addSgoldstinodecayChannels WARNING: channel not configured! Please add also in conf file.\t', dec)
            quit()
        print('channel wanted:',dec)

        if allowed[dec] == 'yes' and wanted[dec] == 'yes':
            
            BR = SG.findBranchingRatio(dec)
            ...
            if 'hadrons' in dec:
                ...
                print("debug readdecay table hadrons BR ",BR)
                #Taking decays from pythia8 Z->qqbar
                #Probably need to change BRZhadtot after I know the branching ratios
                BRZhadtot = 0.6992407
                SGid = P8gen.GetSGId()
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} 1 -1"\
                                    .format(sgid, 0.1540492*BR/BRZhadtot, meMode))
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} 2 -2"\
                                    .format(sgid, 0.1194935*BR/BRZhadtot, meMode))
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} 3 -3"\
                                    .format(sgid, 0.1540386*BR/BRZhadtot, meMode))
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} 4 -4"\
                                    .format(sgid, 0.1193325*BR/BRZhadtot, meMode))
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} 5 -5"\
                                    .format(sgid, 0.1523269*BR/BRZhadtot, meMode))
            else:
                particles = [p for p in dec.replace('->',' ').split()]
                children = particles[1:]
                childrenCodes = [PDGcode(p) for p in children]
                codes = ' '.join(str(code) for code in childrenCodes)
                P8gen.SetParameters("{}:addChannel =  1 {:.12} {} {}"\
                                    .format(P8gen.GetSGId(), BR, meMode, codes))
                print("debug readdecay table ",particles,children,BR)
